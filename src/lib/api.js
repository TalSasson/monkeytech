...
async function fetchCurrentCityWeather(key) {
  let currWeatherInfo = {}
  // const response = await fetch(`${weatherUrl}/currentconditions/v1/${key}?aapikey=awM6pDci3i4lGcDFhVGHqPMLgeAYIehb`)
  ...
  const wearherImage = `${imageUrl}/${WeatherIcon}-s.png`
  currWeatherInfo = {
...
//   console.log('forecast', forecast)
  ...
  const forecastFormat = DailyForecasts.map((day) => ({
    date: day.Date,
    temp: {
  ...
  store.dispatch(setCityDetails('forecast', forecastFormat))
}
